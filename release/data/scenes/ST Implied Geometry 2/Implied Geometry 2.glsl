...
float CROMA = 0.02 + 0.02*(CC[0] + CC[1]*bass);
...
float scene(vec3 p)
{
    ...
    else
    {
        d = min(
            min(
                sdBox(spin2(p), vec3(1.5)*CUBE_SCALE),
                min(
...
void mainImage(out vec4 out_color, vec2 fragCoord)
{
    ...
    //runTime = time + CROMA * texelFetch(iChannel0, ivec2(fragCoord+time)%8, 0).x;
		runTime = CROMA + (bassTime + time)*4.;
    ...
    for(int c=0;c<3;++c)
    {
        ...
        float shakeHtime = runTime + 0.1 * pow(cos(2.1*runTime), 4.);
