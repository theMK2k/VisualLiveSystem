...
vec2 rcpFrame = vec2(1.0/853., 1.0/480.);
vec4 posPos = vec4(gl_TexCoord[0].xy,gl_TexCoord[0].xy - (rcpFrame *0.5));
...
{
	vec4 c = vec4(0.0);
	vec2 rcpFrame = vec2(1.0/1280., 1.0/720.);
...
{ 
	...
	gl_FragColor = vec4(PostFX(lastBuffer, uv, 0.0).rgb, texture2D(lastBuffer, uv).a);
}
