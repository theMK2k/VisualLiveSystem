...
/*
	ST Fractal Flythrough v1.0
	ported from ShaderToy by MK2k https://mk2k.net
	ShaderToy URL:   https://www.shadertoy.com/view/4s3SRN (2018-02-28)
	Original Author: https://www.shadertoy.com/user/Shane

	-- Port Notes --
	Camera Movement is influenced by bassTime

	Strobe effect via godrays.glsl is influenced by bassTime, intensities are customizable with CC[0] and CC[1]
	WARNING: if CC[0] is 0 and CC[1] is 0 or there is no bass, you will have a black screen
*/
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ){
	// Screen coordinates.
//	vec2 u = (fragCoord - iResolution.xy*0.5)/iResolution.y;	// MK2k: this isn't needed in VLS
	vec2 u = (v - v*.5);	// MK2k this seems already correct
    
	float speed = time*0.5 + bassTime + 8.;	// MK2k: replaced iTime with time, added bassTime
