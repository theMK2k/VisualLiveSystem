...
/*
	ST Neon Lit Hexagons v1.0
	ported from ShaderToy by MK2k https://mk2k.net
	ShaderToy URL:   https://www.shadertoy.com/view/MsVfz1 (2018-10-30)
	Original Author: https://www.shadertoy.com/user/Shane
...
float getRndID(vec2 p){
    ...
    float rnd = hash21(p);
    //return smoothstep(.5, .875, sin(rnd*6.283 + time)); // MK2k replaced iTime with time
		return smoothstep(.5, .875, sin(rnd*6.283 + time*0.5 + bassTime + 8.)); // MK2k replaced iTime with time, added bassTime
    ...
    float oGlow = 0.;
		...
		if(rnd>0. && svLitID==1.) {
			...
			ht = floor(ht*4.99)/4./2. + .02;
				...
				// oGlow = mix(1., 0., clamp((abs(p.y - (ht - s)))/s*3.*1.*(1-bass), 0., 1.));
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ){
  ...
  // Screen coordinates.
	...
	vec3 lk = vec3(0, 2.0, (time + bassTime)*8.0); // "Look At" position. // MK2k replaced iTime with time, added bassTime
...
*/  
    ...
    //uv = fragCoord/iResolution.xy;
		//uv = fragCoord/vec2(1920, 1080);	// MK2k: unfortunately, VLS doesn't provide us any resolution info
		//uv = fragCoord/gl_TexCoord[0].xy; // MK2k: this works! but it's not universally applyable
		//uv = v;	// MK2k: doesn't work - it results in checkerboard
		//uv = fragCoord/screen;	// MK2k: doesn't work: black screen
		uv = (v - 1.0) * 2.0;	// as suggested in https://github.com/XT95/VisualLiveSystem/issues/4
