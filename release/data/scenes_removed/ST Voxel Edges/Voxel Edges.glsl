...
float maxcomp( in vec4 k )
{
    return max( max(k.x,k.y), max(k.z,k.w) );
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    //MK2k vec2 mo = iMouse.xy / iResolution.xy;
    //MK2k if( iMouse.w<=0.00001 ) mo=vec2(0.0);
	
	float itime = 2.0*time; //MK2k + 50.0*mo.x;
    // camera
...
{
	float itime = 1.0*time;

    float cr = 0.0;
	vec3 ro = path( itime+0.0, 0.0 ) + vec3(0.0,0.7,0.0);
	vec3 ta = path( itime+2.5, 0.0 ) + vec3(0.0,0.7,0.0);
